...
// 获取会话列表
export const fetchConversations = async (limit = 100, last_id = null, user = 'abc-123') => {
  return get('conversations', { params: { limit, last_id, user } })
}

// 获取特定会话的聊天记录
export const fetchChatList = async ( conversationId: string, limit = 20, first_id = null, user = 'abc-123') => {
  return get('messages', {
    params: { conversation_id: conversationId, limit, first_id, user }
  })
}
